...
from assistant.apertus import DEFAULT_BASE_URL, DEFAULT_MODEL, ApertusModel, system_prompt
...
def test_a_json_reply_becomes_the_proposal():
    model = ApertusModel(client=FakeClient(ONE_RULE))
...
def test_a_reply_wrapped_in_a_code_fence_is_still_read():
    ...
    model = ApertusModel(client=FakeClient(f"Here you go:\n```json\n{ONE_RULE}\n```\n"))
...
def test_recovery_never_discards_a_second_payload_or_truncated_content(suffix):
    model = ApertusModel(client=FakeClient(ONE_RULE + suffix))
...
def test_a_reply_that_is_not_json_proposes_nothing():
    ...
    model = ApertusModel(client=FakeClient("I think you probably want a limit of some kind."))
    assert model.propose(request("at most CHF 50 per order")) == {}
...
def test_background_is_data_and_never_a_system_instruction():
    ...
    ApertusModel(client=client).propose(
...
def test_the_same_words_are_read_the_same_way_twice():
    ...
    client = FakeClient(ONE_RULE)
    ApertusModel(client=client).propose(request("at most CHF 50 per order"))
...
def test_the_customers_turns_reach_the_model_with_their_ids():
    ...
    client = FakeClient(ONE_RULE)
    ApertusModel(client=client).propose(request("only sports shops", "at most CHF 50 per order"))
...
def test_history_reminder_is_the_last_user_message_not_buried_in_background():
    ...
    proposal = PermissionAssistant(ApertusModel(client=client)).draft(turns)
    messages = client.calls[0]["messages"]
...
def test_it_is_the_assistants_model_without_any_change_to_the_assistant():
    ...
    proposal = PermissionAssistant(ApertusModel(client=FakeClient(ONE_RULE))).draft(
...
def test_an_endpoint_failure_is_a_question_not_a_crash():
    ...
    proposal = PermissionAssistant(ApertusModel(client=Broken())).draft(
        request("at most CHF 50 per order").turns)
...
def test_the_key_is_required_before_any_call_is_made(monkeypatch):
    ...
    with pytest.raises(ValueError, match="APERTUS_API_KEY"):
        ApertusModel()


def test_the_endpoint_defaults_to_the_swiss_ai_weeks_one():
    assert DEFAULT_BASE_URL.startswith("https://api.swisscom.com/")
    assert DEFAULT_MODEL.startswith("swiss-ai/Apertus")
...
def test_the_adapter_has_no_import_path_to_the_control_layer_or_its_credential():
    ...
    import assistant.apertus as module
...
def test_a_reply_too_deeply_nested_to_parse_proposes_nothing():
    ...
    bomb = "[" * 30000 + "]" * 30000
    assert ApertusModel(client=FakeClient(bomb)).propose(request("at most CHF 50")) == {}
...
def test_a_product_reference_without_a_catalogue_is_asked_not_crashed():
    ...
    proposal = PermissionAssistant(ApertusModel(client=FakeClient(reply))).draft(
...
def test_the_recorded_prompt_version_is_the_prompt_that_was_actually_sent():
    ...
    from assistant.apertus import prompt_version
...
def test_the_timeout_applies_even_when_the_client_was_built_elsewhere():
    client = FakeClient(ONE_RULE)
    ApertusModel(client=client, timeout_seconds=2.5).propose(request("at most CHF 50 per order"))
...
def test_a_rate_limited_endpoint_is_retried_before_the_customer_sees_a_failure(monkeypatch):
    ...
    assert ApertusModel()._client.max_retries >= 2
