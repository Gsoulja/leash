...
### Laya: separate verification tasks
...
**Build state, 2026-09-25.** LEASH-154, LEASH-101 and LEASH-130 are done — the human gate was cleared for all sixteen review tickets on 2026-09-25. LEASH-174 and LEASH-175 landed in commit `f28f83c`; LEASH-175 is in review with nine of ten criteria met (a German instruction has never been carried through a real model to an enforced verdict), and LEASH-174 still owes three, including the DEC-006 refusal of a loosening rule before it is shown. Step 4 is built: LEASH-146 groups the review under item, price, merchant, frequency and uncertainty headings, names every unrestricted registry field in plain words, and keeps decision codes, catalogue ids, the draft id and the raw payload behind one disclosure — it awaits its own review. Step 3 has the deterministic half of the omission check (the compiler cross-reads the customer's words), but no measured verifier: LEASH-155 has not started, and DEC-058(b) records that the current question fires on readings that are already in `hard_rules`. Step 6 has an idempotent run start; LEASH-102 still owes the customer-authorized backend start and the changed-cart re-evaluation, and LEASH-147 has not started. Nothing here is end-to-end evidenced until LEASH-156 runs.
