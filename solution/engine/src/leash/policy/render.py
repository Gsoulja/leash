...
def permission_review(rules: Sequence[Rule], uncertainty: str) -> dict[str, list[str]]:
    ...
    for rule in rules:
        ...
        if uncertainty == "decline":
            follow.append(f"Decline when triggered: {text}")
        elif uncertainty == "approve" and rule.field == m.F_SESSION_RISK:
            follow.append(f"Session warning alone does not block approval: {text}")
        else:
            ask.append(text)
    if uncertainty == "ask":
        ask.append("Missing or unclear evidence needed to check the rules above.")
    else:
        follow.append(f"When evidence is unclear: {uncertainty}; a missing fact is never recorded as a match.")
    intervention = "Suspected duplicates, off-purpose orders, merchant instructions or integrity problems."
    if uncertainty == "decline":
        follow.append(f"Decline: {intervention}")
    else:
        ask.append(intervention)
    ...
    choose = [f"No explicit restriction on {label}; all other checks still apply."
              for field, label in choices.items() if field not in fields]
    return {"must_follow": list(dict.fromkeys(follow)), "may_choose": choose,
            "must_ask": list(dict.fromkeys(ask))}
