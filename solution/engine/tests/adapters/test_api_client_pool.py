...
from leash.adapters.viseca_api.client import ClientClosed, VisecaClient
...
def test_a_closed_client_refuses_further_requests():
    async def go():
        async with serving() as (_, base):
            ...
            await client.healthz()
            await client.aclose()
            with pytest.raises(ClientClosed, match="closed"):
                await client.healthz()
...
# The environment → Settings → client path is covered in tests/test_config.py: the variables are read
# and validated once, in `Settings`, not by the adapter.
