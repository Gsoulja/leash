...
```bash
python3 "$(git rev-parse --show-toplevel)/.claude/skills/graphloop/graphloop.py" validate --board kanban
```
...
run `python3 "$(git rev-parse --show-toplevel)/.claude/skills/graphloop/graphloop.py" --self-test`. It builds a
...
```bash
python3 "$(git rev-parse --show-toplevel)/.claude/skills/graphloop/graphloop.py" next --board kanban
```
...
```bash
python3 "$(git rev-parse --show-toplevel)/.claude/skills/graphloop/graphloop.py" packet HARNESS-014 --board kanban
```
